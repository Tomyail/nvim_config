...
function M.check_and_switch_input_method()
    ...
    if is_chinese(current_char) or is_chinese(prev_char) or is_chinese(next_char) then
        os.execute("im-select com.apple.inputmethod.SCIM.ITABC") -- Replace "zh" with the appropriate input method code
    else
        os.execute("im-select com.apple.keylayout.ABC") -- Replace "en" with the appropriate input method code
    end
end
